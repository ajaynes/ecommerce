...
import { incrementCart, decrementCart, deleteCartItem, changeQuantityCart } from '../store/cart'

function QuantityInput({ product }) {
    const dispatch = useDispatch();
    const cart = useSelector((state) => state.cart);
    const [quantity, setQuantity] = useState(cart.filter(item => item.id === product.id)[0].quantity)
    const [prevQuantity, setPrevQuantity] = useState(cart.filter(item => item.id === product.id)[0].quantity);
    ...
    return (
        <input type="text" value={quantity} onChange={(handleChange)} onKeyDown={handleEnter} />
    )
...
export default function Cart() {
    ...
    return (
        ...
        <Header />
            <h1>Cart Page</h1>
            {cart.length ?
            <div key={seed}>{cart.map(product => (
                <div key={product.id}>title: {product.title}, qty: {product.quantity}
                    <button onClick={() => increment(product)}>Add</button>
                    <QuantityInput product={product} decrement={() => confirmationDeletion(product)} />
